...
export async function enhanceCompline(priest, full, date){
    ...
    if (season === "PostPentecost" && await isTriodionFeastAfterPentecost(seasonWeek, dayOfWeek) || season === "Pentecost" || season === "EasterWeek" || season === "HolyWeek" || season === "Lent" || season === "Forelent") {
        ...
        if (season === "Pentecost" && seasonWeek === 4 && dayOfWeek === 0) {
            ...
            dayTriodionData["kontakia"] = [
                dayTriodionData["kontakia"],
                (await getData(`${address}\\triodion\\${season}\\44.json`))["kontakia"]
                ]
        }
...
async function selectTropar(season, seasonWeek, dayOfWeek, hourData, glas, dayData, specialDayData, dayTriodionData) {
   ...
   var kontakion = "";
    ...
    if (
        dayTriodionData != undefined && "kontakia" in dayTriodionData
        // Sundays of ascetic saints treated like normal Sundays
        && !(season === "Lent" && (seasonWeek === 4 || seasonWeek === 5) && dayOfWeek === 0)
    ) {
        if (kontakion != "") {
