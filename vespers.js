...
async function loadTextBeginning(vespersData, vespersMenaionData, full, dayOfWeek, mm, dd, season, seasonWeek, glas, dateAddress, dayData, priest, isStBasil, isLenten) {
    ...
    if (season === "PostPentecost" && await isTriodionFeastAfterPentecost(seasonWeek, dayOfWeek) || season === "Pentecost" || season === "EasterWeek" || season === "HolyWeek" || season === "Lent" || season === "Forelent") {
        ...
        try {
            ...
            if (season === "EasterWeek" && (dayOfWeek===0 || dayData["class"]<8)) Object.assign(dayData, dayTriodionData);
        } catch {}
        try {
    ...
    const isEasterWeek = season === "EasterWeek" && dayOfWeek > 0;
    const isPentecostMonday = (vespersTriodionData && "kneeling_prayers" in vespersTriodionData)
    var isGreatVespers = (dayData["class"] >= 8 || dayOfWeek === 0);
    if (isEasterWeek) {
        vespersOctoechosData = await getData(`${address}\\octoechos\\${glas}\\0_vespers.json`);
    } else if (dayOfWeek === 0 && dayData["class"] != 12 || !isGreatVespers) {
    ...
    const isEntrance = (
        ...
        || (
            ...
            || "special_prokimenon_index" in vespersMenaionData
            )
        );
    var priestOrDeacon = "";
    if (priest === "1") {
        // no deacon at vespers if no entrance
        if (isEntrance) priestOrDeacon = priestlyExclamationsData["deacon"];
        else priestOrDeacon = priestlyExclamationsData["priest"];
    }

    var vigilVespersData, haire;
    if (season === "EasterWeek" && dayOfWeek > 0) {
        document.getElementById("beginning").innerHTML = `${getBeginning(priest)}<br><br>`;
    } else if (dayData["class"] >= 10) {
        vigilVespersData = await getData(`${address}\\horologion\\vigil_vespers.json`);
        haire = vigilVespersData["haire"];
        var text = "";
        if (priest == "1") {
            text += `${vigilVespersData["beginning"]}<br><br>${amen}<br><br>`
        } else {
        ...
        let alternative = ""
        if (season === "Pentecost" && seasonWeek === 5 && dayOfWeek === 3) {
        ...
        document.getElementById("beginning").innerHTML = `
          ...
          <hr>
          ${await usualBeginning(priest, season, seasonWeek, dayOfWeek, true)}
          <hr>
    ...
    if (isGreatVespers && !isEasterWeek) {
      ...
      document.getElementById("psalm103Selector").addEventListener("change",() => makePs103(isGreatVespers, isEasterWeek, priest));
    } else {
        document.getElementById("psalm103Selector").innerHTML = ""
    }
    makePs103(isGreatVespers, isEasterWeek, priest);
    ...
    if (priest == "1"){
        ...
        document.getElementById("priestly_prayers_selector").innerHTML = `
          ...
          <br><br>`
        if (isPentecostMonday) {priestPrayers["light"].splice(7, 1);}
        ...
        if ("special_prokimenon_index" in vespersMenaionData && vespersMenaionData["special_prokimenon_index"] === "deceased") {
            document.getElementById("ektenia_peace").innerHTML = makeEktenia(ekteniaData["deceased"], priestOrDeacon);
        } else if (isPentecostMonday) {
            var ektenia_peace = ekteniaData["peace"];
            ektenia_peace = (
                ektenia_peace.slice(0, ektenia_peace.length - 4)
                .concat(vespersTriodionData["ektenia_peace"])
                .concat(ektenia_peace.slice(ektenia_peace.length - 4, ektenia_peace.length))
            );
            document.getElementById("ektenia_peace").innerHTML = makeEktenia(ektenia_peace, priestOrDeacon);
        } else {
            document.getElementById("ektenia_peace").innerHTML = makeEktenia(ekteniaData["peace"], priestOrDeacon);
        }
    } else {
        document.getElementById("priestly_prayers_selector").innerHTML = "";
        document.getElementById("priestly_prayers").innerHTML = "";
    ...
    if (dayData["class"] === 12 && dayOfWeek >= 2 || omit_kathisma || dayOfWeek === 1 && dayData["class"] < 8) {
        ...
        document.getElementById("kathismaSelector").innerHTML = `
          <label><input type="radio" name="kathismaChoice" value="omit_kathisma" checked> Omit kathisma</label><br>
          <label><input type="radio" name="kathismaChoice" value="full_kathisma"> Full kathisma</label>
        `
    }

    makeKathisma(dayOfWeek, dayData["class"], mm, dd, season, seasonWeek, priest, priestOrDeacon, ekteniaData, omit_kathisma);
    document.getElementById("kathismaSelector").addEventListener("change",() => makeKathisma(dayOfWeek, dayData["class"], mm, dd, season, seasonWeek, priest, priestOrDeacon, ekteniaData, omit_kathisma));
    ...
    if (priest === "1"){
        var wisdom = `${priestOrDeacon} ${priestlyExclamationsData["wisdom"]}`;
    } else {
        wisdom = "";
    }
    makeHymnOfLight(priest, isEntrance, priestPrayers, wisdom, vespersData);

    var prokimenon = await makeProkimenon(dayOfWeek, vespersData, priest, priestOrDeacon, dayData, vespersMenaionData, priestlyExclamationsData, vespersTriodionData, isEasterWeek);
    if (priest === "1" && isPentecostMonday) {
        var ektenia_small = ekteniaData["small"];
        ektenia_small[3] = vespersTriodionData["kneeling_prayers"][4];
        prokimenon += `
            <br>${vespersTriodionData["kneeling_prayers"][0]}<br><br>
            <span style="color:red;">Choir:</span> ${LHM} <span style="color:red;">(3)</span><br><br>
            ${vespersTriodionData["kneeling_prayers"][1]}<br><br>
            ${makeEktenia(ektenia_small, priestOrDeacon)}
            `
    }
    document.getElementById("prokimenon").innerHTML = prokimenon;

    if ("readings" in vespersMenaionData || vespersTriodionData != undefined && ("readings" in vespersTriodionData || "gospel" in vespersTriodionData)){
    ...
    if (isStBasil && priest === "1" && (isWeekday || season === "EasterWeek")) {
        ...
        var doxology = await lesserDoxology("vespers", season === "Lent" && dayOfWeek != 0 && dayOfWeek < 6 && dayData["class"] < 8);
        if (priest === "1" && isPentecostMonday) {
            var ektenia_small = ekteniaData["small"];
            ektenia_small[3] = vespersTriodionData["kneeling_prayers"][6];
            doxology += `
                <br><br>${vespersTriodionData["kneeling_prayers"][0]}<br><br>
                <span style="color:red;">Choir:</span> ${LHM} <span style="color:red;">(3)</span><br><br>
                ${vespersTriodionData["kneeling_prayers"][3]}<br><br>
                ${makeEktenia(ektenia_small, priestOrDeacon)}
                `
        }
        document.getElementById("lesserDoxology").innerHTML = doxology;
    }

    if (isGreatVespers || isPentecostMonday) {
        if (isStBasil && priest === "1" && (isWeekday || season === "EasterWeek") || isLenten) {
            document.getElementById("ektenia_augmented_great").innerHTML = "";
        } else if (priest === "1") {
            var augmented = `
                ${makeEktenia(ekteniaData["pre_augmented"], priestOrDeacon, "short") + makeEktenia(ekteniaData["augmented"], priestOrDeacon, "augmented")}
                <br><br>`
            if (isPentecostMonday) {
                var ektenia_small = ekteniaData["small"];
                ektenia_small[3] = vespersTriodionData["kneeling_prayers"][5];
                augmented += `
                    ${vespersTriodionData["kneeling_prayers"][0]}<br><br>
                    <span style="color:red;">Choir:</span> ${LHM} <span style="color:red;">(3)</span><br><br>
                    ${vespersTriodionData["kneeling_prayers"][2]}<br><br>
                    ${makeEktenia(ektenia_small, priestOrDeacon)}<br><br>
                `
            }
            document.getElementById("ektenia_augmented_great").innerHTML = augmented;
        } else {
    ...
    if (isStBasil && priest === "1" && (isWeekday || season === "EasterWeek")) {
        document.getElementById("ektenia_augmented_great").innerHTML = "";
         ...
         var ektSupp = makeEktenia(ekteniaData["supplication"], priestOrDeacon, "supplication");
         ...
         ${priestOrDeacon} ${priestlyExclamationsData["bow"]}<br>
         ${TYL}<br><br>
...
async function loadTextEnding(vespersData, dayOfWeek, mm, dd, season, seasonWeek, glas, dayData, vespersMenaionData, priest, isLenten){
    ...
    if (isLytia) {
        if (mm === 3 && dd === 25 && dayOfWeek != 0 && dayOfWeek != 6 && (season === "Lent" || season === "HolyWeek")) {
            // Glory is not said unless it's Sat or Sun
    ...
    if (isLenten || isSemiLenten) {
        document.getElementById("ektenia_augmented_or_ps33").innerHTML = await makeLentenEnding(priest, season, seasonWeek, dayOfWeek, dayData, ekteniaData);
    } else if (isEasterWeek) {
        document.getElementById("ektenia_augmented_or_ps33").innerHTML = "";
    } else if (!isGreatVespers && !isLenten && !isPentecostMonday) {
        if (priest == "1"){
            document.getElementById("ektenia_augmented_or_ps33").innerHTML = makeEktenia(ekteniaData["augmented"], priestOrDeacon, "augmented") + "<br><br>";
        } else {
    ...
    if (season === "Lent" && seasonWeek === 4 && dayOfWeek > 0 && dayOfWeek <= 5) {
        ...
        document.getElementById("after_hour_elements").innerHTML = `
            <div class="rubric">
                The Cross is now transferred from the sacristy to the altar.
            </div><br>`;
    } else if (isEasterWeek) {
        var roles;
...
export async function makePs33(priest, vigilVespersData){
    ...
    text += `${cross} ${vigilVespersData["blessed_be"]} <FONT COLOR="RED"> (3)</FONT><br><br>`
...
async function makeLytiaPrayers(lytiaPrayers, vigilVespersData, vespersData, dayData, priestlyExclamationsData){
    ...
    var lytia = `
        ...
        ${priestlyExclamationsData["deacon"]} ${priestlyExclamationsData["bow"]}<br><br>
...
export async function makeEndingBlockMajor(priest, priestOrDeacon, season, seasonWeek, dayOfWeek, isGreatVespers, vespersData, dayData, priestlyExclamationsData, isLenten, isEasterWeek) {
    ...
    if (priest === "1"){
        if (greatDismissal) res += `
            ${priestOrDeacon} ${priestlyExclamationsData["wisdom"]}<br><br>
...
async function makeLentenEnding(priest, season, seasonWeek, dayOfWeek, dayData, ekteniaData) {
    ...
    if ("no_kathisma" in dayData) dayClass = 11;
    // possible cases:
    ...
    // 4) no Ephrem nor conclusion (Fri evenings Triodion p149 of pdf/Dol 398, feast on Mon 266, Annunciation p 284)
    ...
    if (!noEphrem && !smallEphrem || dayOfWeek === 1 && dayClass < 8) {
        ...
        if (priest == "1"){
            text = makeEktenia(ekteniaData["augmented"], priestOrDeacon, "augmented") + "<br><br>" + text;
        } else {
...
export async function makeTroparia(glas, season, seasonWeek, dayOfWeek, isGreatVespers, dayData, haire, specialSundayName, isLenten){
    if (isLenten && dayOfWeek < 6 && !("no_kathisma" in dayData)) {
    ...
    } else if (season === "Pentecost" && (seasonWeek === 5 && dayOfWeek > 4 || seasonWeek === 6 && dayOfWeek <= 5)) {
        // Ascension post-feast
        theotokion = (await getData(`${address}\\triodion\\${season}\\44.json`))["troparia"];
        if (dayOfWeek === 5) dayTrop = [];
...
async function makeGloryAposticha(aposticha, vespersMenaionData, dayOfWeek, dayClass) {
    ...
    var tone;
    var i = 0;
    for (let [j, stychera] of apostMen.entries()){
        if (!isNaN(parseInt(stychera[0]))){
            // this is tone indication
            tone = stychera;
            continue
...
export async function makeAposticha(glas, season, seasonWeek, dayOfWeek, isGreatVespers, dayData, vespersData, vespersMenaionData, vespersOctoechosData, vespersTriodionData){
    ...
    var useSunday = (dayOfWeek === 0 && dayData["class"] != 12 && !("label" in dayData && dayData["label"] === "24") && season != "Pentecost");
    if (useSunday) {
    ...
    } else if (season === "Pentecost" && seasonWeek >= 2 && seasonWeek < 6 && dayOfWeek === 0) {
        ...
        apostVerses = [
            ...
            versesMaterial[4],
            versesMaterial[5],
            `<div class="rubric">Tone ${vespersTriodionData["aposticha"][0]}</div>${glory}`,
            andNow];
    } else if (season === "Pentecost" && (seasonWeek < 5 || seasonWeek === 5 && dayOfWeek < 4) && dayOfWeek > 0 && dayData["class"] >= 8) {
        ...
        apostMain.splice(4, 2); // rm glory and tone
    } else if (season === "Pentecost" && dayOfWeek > 0 && !(seasonWeek === 6 && dayOfWeek > 4) && dayData["class"] >= 8) {
        // feast on Ascensiontide
        apostMain = vespersMenaionData["aposticha"].concat(vespersTriodionData["aposticha"][6]);
        apostVerses = vespersMenaionData["aposticha_verses"].concat(
            [`<div class="rubric">Tone ${apostMain[4]}</div>${glory}`,
        ...
        // mi-pentecost or Leave-taking
        if (seasonWeek === 4) vespersTriodionData = (await getData(`${address}\\triodion\\Pentecost\\23_vespers.json`));
        apostMain = vespersTriodionData["aposticha"];
        apostVerses = vespersTriodionData["aposticha_verses"].concat(
        ...
        const apostSun = (await getData(`${address}\\octoechos\\${glas}\\0_vespers.json`));
        if (dayOfWeek === 1 || dayOfWeek === 2) {
            apostMain = [apostMain[0], apostSun["ps140"][dayOfWeek], apostMain[dayOfWeek], apostMain[3]].concat(vespersTriodionData["aposticha"][2])
            ...
            apostVerses = [
                ...
                versesMaterial[4],
                versesMaterial[5],
                `<div class="rubric">Tone ${vespersTriodionGN[vespersTriodionGN.length-3]}</div>${gloryAndNow}`,
                ""
            ];
            apostMain = (
                apostSun["ps140"].slice(0, 4)
                .concat(apostSun["aposticha"][1])
        ...
        season === "Pentecost" && (seasonWeek >= 5 && !(seasonWeek === 5 && dayOfWeek < 4)) && dayData["class"] < 8
         ...
         // postfeasts of Ascension and Pentecost
         apostMain = vespersTriodionData["aposticha"].slice(0, 4);
         apostVerses = vespersTriodionData["aposticha_verses"];
         if ("aposticha" in vespersMenaionData && !(seasonWeek === 7 && dayOfWeek === 1)) {
            apostMain = apostMain.concat([vespersMenaionData["aposticha"][2], vespersTriodionData["aposticha"][6]])
            apostVerses = apostVerses.concat(
                [`<div class="rubric">Tone ${vespersMenaionData["aposticha"][0]}</div>${glory}`,
                `<div class="rubric">Tone ${vespersTriodionData["aposticha"][4]}</div>${andNow}`]
            );
         } else if (seasonWeek === 6 && dayOfWeek === 1) {
    ...
    if (season === "EasterWeek" && dayOfWeek > 0) {
        ...
        if (dayData["class"] >= 8) {
            const apostMen = vespersMenaionData["aposticha"];
            apostVerses = apostMain["aposticha_verses"].concat(
                [`<div class="rubric">Tone ${apostMen[apostMen.length-3]}</div>${glory}`,
                `<div class="rubric">Tone ${apostMain["aposticha"][0]}</div>${andNow}`]
            );
            apostMain = (
                apostMain["aposticha"].slice(0, 5)
                .concat(apostMen[apostMen.length-1])
                .concat(apostMain["aposticha"][5])
            )
        } else {
             apostVerses = apostMain["aposticha_verses"].concat([gloryAndNow]);
             apostMain = apostMain["aposticha"];
        }
        ...
        aposticha += `
            ...
            `
        for (let [i, stychera] of apostMain.slice(1, 10).entries()) {
            aposticha += `<i>${apostVerses[i]}</i> <br><br>${stychera}`
            if (i < apostMain.length-2) aposticha += "<br><br>"
        }
        aposticha += ` <FONT COLOR="RED">(3)</FONT><br><br>`
        if (vespersTriodionData && "aposticha_rubric" in vespersTriodionData) aposticha += `<div class="rubric">${vespersTriodionData["aposticha_rubric"]}</div>`
    } else if (season === "Pentecost" && !(seasonWeek===1 && dayOfWeek===0)) {
        aposticha += `
            <div class="rubric">Tone ${apostMain[0]}</div>
            `
        for (let [i, stychera] of apostMain.slice(1, apostMain.length).entries()) {
            aposticha += `${stychera}<br><br>`
            if (i < apostMain.length-2) aposticha += `<i>${apostVerses[i]}</i><br><br>`;
        }
    } else if (
        !isGreatVespers
        && prePostFeast === ""
        ...
        if ("aposticha" in vespersMenaionData && (vespersTriodionData === undefined || dayData["class"] >= 8)) {
            ...
            aposticha += `<div class="rubric">Tone ${tone}</div>`;
            if (vespersTriodionData["aposticha"][1] === "gn") {
                aposticha += `<i>${gloryAndNow}</i><br><br>${vespersTriodionData["aposticha"][2]}<br><br>`
            } else {
...
function makeReadings(vespersMenaionData, priest, dayOfWeek, ekteniaData) {
    var text = "";
    if (!vespersMenaionData) return text;
    if (vespersMenaionData && ("readings" in vespersMenaionData)) text += frameReadings(vespersMenaionData["readings"]);
    ...
    if ("gospel" in vespersMenaionData) {
        ...
        if (gospelData.length === 2) {
            ...
            text += `<div class="rubric">Tone ${gospelData[0][0]}</div>`;
            text += `${alleluia}<br>`;
            for (let verse of gospelData[0].slice(1, gospelData[0].length)) {
                text += `<FONT COLOR="RED">v.</FONT> ${verse} <br>
                    <FONT COLOR="RED">Choir:</FONT> ${alleluia}<br>`;
            }
            text += "<br>"
        }
        text += frameReadings(gospelData[gospelData.length-1]);
    }
    if ("troparia_and_readings" in vespersMenaionData) {
        ...
        if (priest === "1") {
            text += `${makeEktenia(ekteniaData["small"], priestOrDeacon)}<br><br>`;
...
async function makeProkimenon(dayOfWeek, vespersData, priest, priestOrDeacon, dayData, vespersMenaionData, priestlyExclamationsData, vespersTriodionData, isEasterWeek) {
    if (dayData["class"] === 14) return ""; // Holy Sat
    var prokimenon = `<div class="subhead">Prokimenon</div><br>`;
    if (priest === "1"){
        prokimenon += `
        ${priestOrDeacon} ${priestlyExclamationsData["attentive"]}<br>
        ${priestlyExclamationsData["peace"]}<br>
        ${priestOrDeacon} ${priestlyExclamationsData["wisdom"]} ${priestlyExclamationsData["stand"]}<br><br>`
    }
    var prokData;
    ...
    if ("special_prokimenon_index" in vespersMenaionData && (
        ...
        prokData = (await getData(`${address}\\horologion\\special_prokimena.json`))[dayOfWeek-1];
    } else if (vespersTriodionData != undefined && "special_prokimenon_index" in vespersTriodionData) {
...
function makeHymnOfLight(priest, isEntrance, priestPrayers, wisdom, vespersData){
    if (!isEntrance || priest === "0") {
...
async function makePsalm140(dayOfWeek, season, seasonWeek, glas, isGreatVespers, vespersData, vespersMenaionData, vespersOctoechosData, vespersTriodionData, dayData, specialSundayName){
    ...
    if (season === "EasterWeek" && seasonWeek === 1 && dayOfWeek === 0) {
        // holy Sat
        forceNumSticheras = 8;
        ...
        stycheraScheme[4] = 2;
    } else if (season === "EasterWeek" && dayData["class"] === 8) {
        // polyeleos in EW, rubrics from Pochaiv Menaion st George
        forceNumSticheras = 6;
        theotokiaData = (await getData(`${address}\\octoechos\\${glas}\\stychera_theotokia.json`))[0];
        stycheras = (
            vespersOctoechosData["ps140"].slice(0, 4)
            .concat(vespersMenaionData["ps140"])
            .concat(["n", vespersOctoechosData["ps140"][0], theotokiaData])
        )
        numStycheras = 6;
        stycheraScheme = Array(numStycheras).fill(1);
    } else if (season === "EasterWeek") {
        forceNumSticheras = 6;

        stycheras = vespersOctoechosData["ps140"]
    ...
    } else if (season === "PostPentecost" && dayData["class"] === 11 && dayOfWeek != 0 && vespersTriodionData != undefined) {
        // compassion
        stycheras = psalm140menaionStycheras;
        ...
        // Fathers of Nicaea
        if (dayData["class"] < 8) {
            const prePostFeastData = (await getData(`${address}\\triodion\\${season}\\44_vespers.json`));
            stycheras = (
                vespersOctoechosData["ps140"].slice(0, 4)
        ...
        // mid-Pentecost or leave-taking
        if (seasonWeek === 3) stycheras = vespersTriodionData["ps140"];
        else {
            stycheras = (await getData(`${address}\\triodion\\${season}\\23_vespers.json`))["ps140"];
        }
        numStycheras = 3;
        stycheraScheme = Array(3).fill(2);
    } else if (season === "Pentecost" && seasonWeek === 6 && dayOfWeek === 5) {
         stycheras = (await getData(`${address}\\triodion\\${season}\\44_vespers.json`))["ps140"];
         numStycheras = 5;
        ...
        forceNumSticheras = 8;
        numStycheras = 3;
    } else if (season === "Lent" && "no_kathisma" in dayData && dayOfWeek > 0 && dayOfWeek < 6) {
        // March 26 on weekday of Lent
        stycheras = vespersTriodionData["ps140"].concat(psalm140menaionStycheras);
        // 3 tiod + 4 feast + 3 achrangel
        stycheraScheme = Array(3).fill(1).concat([2]).concat(Array(5).fill(1));
        forceNumSticheras = 10;
        numStycheras = numTriodionStycheras + numStycheras;
        ...
        numStycheras = 6;
        forceNumSticheras = 6;
    } else if (dayOfWeek > 0 && dayOfWeek < 6 && season === "Lent" && dayData["class"] >= 11) {
        // Annunciation on weekay of Lent
        stycheras = vespersTriodionData["ps140"].concat(psalm140menaionStycheras)
        // 3 triodion, 5 feast
        ...
        if (dayData["class"] >= 8) {
            ...
            // Sun of triod, no poly, no pre/post feast
    ...
    // triodion + polyeleos below
        ...
        if (prePostFeast != "" && dayData["class"] >= 8) {
            ...
            numStycheras = 9
        } else if (prePostFeast === "postfeast" && "troparia" in dayData && dayData["troparia"].length === 0 && !("no_kathisma" in dayData)) {
            // leave-taking on Sun
            stycheras = psalm140OctoechosStycheras.slice(0, 5).concat(psalm140menaionStycheras)
            // 4 + 6
            stycheraScheme = Array(numStycheras).fill(Math.floor(6 / numStycheras));
    ...
    if (!theotokionWasAdded) {
        ...
        var theotokiaData;
        if (dayOfWeek != 0 && dayOfWeek != 6 && isGreatVespers || season === "EasterWeek"){
...
function makeSmallEktenia(priest, priestOrDeacon, ekteniaData){
    // add small ektenia
    if (priest == "1"){
        document.getElementById("ektenia_small").innerHTML = makeEktenia(ekteniaData["small"], priestOrDeacon);
    } else {
...
async function makeKathisma(dayOfWeek, dayClass, mm, dd, season, seasonWeek, priest, priestOrDeacon, ekteniaData, omit_kathisma){
    ...
    if (instruction === "verses" && !omit_kathisma) {
        ...
        makeSmallEktenia(priest, priestOrDeacon, ekteniaData);
        return;
    }
...
export function makeEktenia(ekteniaData, priestOrDeacon, key=false){
    var ektenia = priestOrDeacon + " ";
...
async function makePs103(great, isEasterWeek, priest) {
    ...
    if (isEasterWeek) {
        ...
        var roles;
        if (priest === "1") roles = [`<FONT COLOR="RED">Priest:</FONT>`, `<FONT COLOR="RED">Choir:</FONT>`]
        else roles = [`<FONT COLOR="RED">1:</FONT>`, `<FONT COLOR="RED">2:</FONT>`]
        ...
        if (priest === "0") roles = [`<FONT COLOR="RED">v.</FONT>`, `<FONT COLOR="RED">Choir:</FONT>`]
        ...
        document.getElementById("psalm103").innerHTML = res;
    } else if (instruction === "full") {
